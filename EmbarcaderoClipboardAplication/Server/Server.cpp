...
{
Memo1->Lines->Clear(); //Pregatim logs memo
...
//Serverul nu functioneaza fara clienti.
    ...
    if(ServerSocket->Socket->ActiveConnections==0) if(Backup.is_open()) Backup.close();
}
//---------------------------------------------------------------------------
void __fastcall TClipboardServer::ServerSocketClientRead(TObject *Sender, TCustomWinSocket *Socket)
{
	...
	String user; //Useru local ptr folosire in event
	 ...
	 if(Backup.is_open())
	 {
	 AnsiString textBackup=Server->getText();
     Backup << textBackup << endl;
     }
